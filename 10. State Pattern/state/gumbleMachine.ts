...
export class GumballMachine {
  static noQuarterState: State;
  static hasQuarterState: State;
  static soldState: State;
  static soldOutState: State;
  ...
  constructor(count: number) {
    ...
    if (count > 0) {
      ...
      this.state = GumballMachine.noQuarterState;
    }
  }
  ...
  // 동전 투입
  insertQuarter(): void {
    if (this.state === GumballMachine.hasQuarterState) {
      console.log('동전은 한 개만 넣어 주세요.'); // 이미 동전이 투입되어 있음
    } else if (this.state === GumballMachine.noQuarterState) {
      this.state = GumballMachine.hasQuarterState;
      console.log('동전을 넣으셨습니다.'); // 동전을 받은 다음 hasQuarterState 상태로 변경
    } else if (this.state === GumballMachine.soldOutState) {
      console.log('매진되었습니다. 다음 기회에 이용해 주세요.'); // 매진 상태에서는 뽑을 알맹이가 없으므로 동전을 반환
    } else if (this.state === GumballMachine.soldState) {
  ...
  turnCrank(): void {
    if (this.state === GumballMachine.hasQuarterState) {
      ...
      this.state = GumballMachine.soldState;
      this.dispense();
    } else if (this.state === GumballMachine.noQuarterState) {
  ...
  dispense(): void {
    if (this.state === GumballMachine.hasQuarterState) {
      ...
      if (this.count === 0) {
        ...
        this.state = GumballMachine.soldOutState;
      } else {
        this.state = GumballMachine.noQuarterState;
      }
    }
